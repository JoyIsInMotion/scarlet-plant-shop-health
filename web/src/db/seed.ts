...
const ACCESSORY_DESCRIPTIONS: Array<{ match: string; en: string; bg: string }> = [
  { match: 'Plant Pot',       en: 'Durable ceramic plant pot with drainage hole — keeps roots healthy and soil aerated.',                                                             bg: 'Издръжлива керамична саксия с дренажен отвор — пази корените здрави и почвата проветрена.' },
  { match: 'Soil Mix',        en: 'Premium potting soil with perlite for excellent drainage and nutrient retention. Ready to use straight from the bag.',                             bg: 'Висококачествена почвена смес с перлит за отличен дренаж и задържане на хранителни вещества. Готова за директна употреба.' },
  { match: 'Liquid Fertilizer', en: 'Balanced liquid fertilizer to encourage lush growth and vibrant blooms. Dilute in water before applying.',                                      bg: 'Балансиран течен тор за буен растеж и ярко цъфтене. Разредете с вода преди употреба.' },
  { match: 'Watering Can',    en: 'Ergonomic watering can with a slender spout for precise, root-level watering — ideal for indoor plants.',                                          bg: 'Ергономична лейка с тесен накрайник за прецизно поливане на нивото на корените — идеална за стайни растения.' },
  { match: 'Plant Label',     en: 'Waterproof plant labels for keeping track of your collection. Easy to write on and wipe clean for reuse.',                                         bg: 'Водоустойчиви етикети за идентифициране на растенията ви. Лесни за надписване и повторна употреба.' },
  { match: 'Pebble Tray',     en: 'Decorative pebble tray placed under your pot to boost humidity and catch excess drainage water.',                                                  bg: 'Декоративна тавичка с камъчета под саксията за повишаване на влажността и задържане на излишната вода.' },
  { match: 'Spray Bottle',    en: 'Fine-mist spray bottle for misting humidity-loving plants and gently cleaning dusty leaves.',                                                      bg: 'Пулверизатор с фина мъгла за напрашване на влаголюбиви растения и нежно почистване на листата.' },
  { match: 'Pruning Shears',  en: 'Sharp stainless-steel pruning shears for clean, precise cuts that help your plants heal faster.',                                                  bg: 'Остри ножици от неръждаема стомана за чисти и прецизни срезове, ускоряващи зарастването.' },
  { match: 'Hanging Planter', en: 'Stylish hanging planter that brings vertical greenery to any room. Suitable for trailing plants.',                                                 bg: 'Стилна висяща кашпа за вертикално озеленяване. Подходяща за висящи растения.' },
  { match: 'Pot Saucer',      en: 'Matching ceramic saucer to protect your surfaces from drainage water while keeping your setup tidy.',                                              bg: 'Подходяща керамична чиния за защита на повърхностите от дренажна вода — поддържа реда.' },
];
...
async function fixProductDescriptions() {
  const rows = await db
    .select({ id: schema.products.id, nameEn: schema.products.nameEn })
    .from(schema.products)
    .where(eq(schema.products.category, 'accessories'));
  ...
  console.log(`🛠  Fixed ${updated} accessory descriptions`);
}

async function fixBgNames() {
  await sql`
  ...
  `;
  console.log('🌿 Capitalized BG species names');
}
