...
function EditPlantContent({ id }: { id: string }) {
  ...
  useEffect(() => {
    ...
    authedRequest((token) => getPlant(id, token))
      .then((p) => {
        if (!active) return;
        setPlant(p);
        setName(p.customName);
        setLastWatered(toDateString(p.lastWatered));
        setSpeciesId(p.speciesId ?? null);
        setSpeciesConfirmed(p.speciesConfirmed ?? false);
        if (p.species) {
          setSelectedSpecies({
            id: p.speciesId!,
            commonNameEn: p.species.commonNameEn ?? null,
            commonNameBg: p.species.commonNameBg ?? null,
            scientificName: p.species.scientificName,
          });
        }
      })
      .catch(() => {})
      .finally(() => { if (active) setLoadingPlant(false); });
    return () => { active = false; };
  }, [authedRequest, id]);
  ...
  const currentPhoto = image?.uri ?? plant?.imageUrl;

  async function save() {
    const trimmedName = name.trim();
    if (!trimmedName) { setNameError('Required'); return; }
    setNameError('');
    setSaving(true);
    try {
      ...
      if (image) {
        try {
          await authedRequest((token) => uploadPlantImage(id, image, token));
        } catch {
          notify(
            locale === 'en' ? 'Saved' : 'Запазено',
            locale === 'en' ? 'Changes saved — but the photo failed to upload.' : 'Промените са запазени, но снимката не се качи.'
          );
      ...
      notify(m.common.error, e instanceof ApiError ? e.message : m.plants.editFailed);
    } finally {
      setSaving(false);
    }
  }
  ...
  const goBack = useCallback(
    () => (router.canGoBack() ? router.back() : router.replace(`/plants/${id}`)),
    [router, id]
  );
        ...
        <ScrollView
          ...
          <View style={styles.card}>
            ...
            <Pressable
              ...
              {currentPhoto ? (
                <Image source={{ uri: currentPhoto }} style={styles.photoPreview} contentFit="cover" />
              ) : (
            ...
            {!identifying && identification?.identified && identification.matchedSpeciesId && !speciesConfirmed && (
              <View style={styles.identifyCard}>
                ...
                <View style={styles.identifyBtnRow}>
                  <Pressable onPress={confirmSpecies} style={({ pressed }) => [styles.confirmBtn, pressed && styles.pressed]}>
                    <Text style={styles.confirmBtnText}>✓ {m.plants.confirmSpecies}</Text>
                  </Pressable>
                  <Pressable onPress={rejectSpecies} style={({ pressed }) => [styles.rejectBtn, pressed && styles.pressed]}>
          ...
          <View style={styles.btnRow}>
            ...
            <Pressable
              ...
              {saving ? (
                ...
                <Text style={styles.saveBtnText}>{m.plants.saveChanges}</Text>
              )}
            </Pressable>
          </View>
        </ScrollView>
...
const styles = StyleSheet.create({
  ...
  searchInput: { flex: 1, paddingVertical: 13, fontSize: 15, color: COLORS.foreground },
